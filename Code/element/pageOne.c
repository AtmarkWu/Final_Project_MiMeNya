...
Elements *New_PageOne(int label)
{
    ...
    pDerivedObj->Content[0] = al_load_bitmap("assets/image/Book/book_cat1.png"); //縮圖
    ...
    pDerivedObj->Content[1] = al_load_bitmap("assets/image/Book/book_cat2.png");
    ...
    pDerivedObj->GetSign[0] = al_load_bitmap("assets/image/Book/book_no1mi.png");
    pDerivedObj->GetSign[1] = al_load_bitmap("assets/image/Book/book_no2mi.png");
    pDerivedObj->NewSign = al_load_bitmap("assets/image/Book/New.png");

    //細節圖背景設定
    pDerivedObj->Detail[0] = al_load_bitmap("assets/image/Book/no1_mi.png");
    pDerivedObj->Detail[1] = al_load_bitmap("assets/image/Book/no2_mi.png");
...
void PageOne_draw(Elements *self)
{
    if(gameFunction == 0 && Page == 1){ //如果在圖鑑功能內且頁數為1
        ...
        int gap = 250;
        //畫出預視圖框框
        for(int i = 0 ; i < FrameUD ; i++){ //索引值對應種類從1開始
            al_draw_bitmap(Obj->decFrame[i], Obj->x[i], Obj->y[i], 0);
            //畫出要顯示在上面的貓咪縮圖、資訊等等
            if(Own[i+1]){ //如果是已經擁有的貓 -> 顯示縮圖
        ...
        for(int i = 0 ; i < FrameUD ; i++){
            ...
            if(Obj->Open[i] && Obj->current_open == i && Own[i+1]){ 
                //畫出該細節圖應該要有的內容(主圖、貓咪、介紹、持有數等等)
                al_draw_bitmap(Obj->Detail[i], 0, 0, 0);
                //呼叫函式畫出持有數
                HowManyCatIHave(self, CatNumber[i+1]);
                //printf("---> %d\n", CatNumber[0]);
                if(NewCatOrNot[i+1]){ //如果該貓咪是新的 -> 點進來後已經看過內容(不再是新的)
                    // -> 在預視圖要改成顯示get
                    NewCatOrNot[i+1] = false;
                }
            }
        }
